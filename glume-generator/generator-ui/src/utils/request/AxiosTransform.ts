...
export class AxiosTransformImpl implements AxiosTransform {
  ...
  requestInterceptors(config: RequestConfigWithOptional, options: IAxiosRequestConfig): RequestConfigWithOptional {
    ...
    if (token && config?.requestOptions?.withToken !== false) {
      ...
      config.headers[fieldToken] = options.authenticationScheme ? `${options.authenticationScheme} ${token}` : token;
    }
    // 请求防重提交
    if (
      options.requestOptions.isRepeatSubmit &&
      ...
      const requestObj = {
        url: config.url,
        data:
          Object.prototype.toString.call(Number(config.data)).slice(8, -1) === 'Object'
            ? JSON.stringify(config.data)
            : config.data,
        params:
          Object.prototype.toString.call(Number(config.params)).slice(8, -1) === 'Object'
