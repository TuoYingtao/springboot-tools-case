...
export default defineComponent({
  ...
  setup(props) {
    ...
    const renderBreadcrumbItem = () => levelList.value.map((item, index) => <ElBreadcrumbItem key={item.path} v-slots={{
      default: renderContext(item, index)
    }} />)

    const renderTransitionGroup = () => <TransitionGroup name="breadcrumb" v-slots={{
