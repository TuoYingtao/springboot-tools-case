...
export default defineComponent({
  ...
  setup(props) {
    ...
    const renderTag = (item: DictInfo, index: number) => <ElTag disableTransitions={true} key={item.value} class={item.elTagClass}>{item.label + " "}</ElTag>
        ...
        (props.options as DictInfo[]).map((item: DictInfo, index) => {
          if (values.value.includes(item.value)) {
            if ((item.elTagType == 'default' || item.elTagType == '') && (item.elTagClass == '' || item.elTagClass == null)) {
              return (<span key={item.value} class={item.elTagClass}>{item.label + " "}</span>);
            } else {
