...
export default defineComponent({
  ...
  setup(props) {
    ...
    const renderSidebarItem = () => sidebarRouters.value.map((route, index: number) => {
      return (<SidebarItem key={route.path + index} item={route} basePath={route.path} />)
    })

    const renderMenu = () => <ElMenu defaultActive={activeMenu.value} collapse={isCollapse.value}
    ...
    const renderScrollbar = () => <ElScrollbar class={sideTheme.value} wrapClass="scrollbar-wrapper" v-slots={{
      default: renderMenu
    }} />
