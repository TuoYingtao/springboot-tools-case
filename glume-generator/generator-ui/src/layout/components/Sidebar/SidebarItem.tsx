...
export default defineComponent({
  ...
  setup(props) {
    ...
    const renderMenuTitle = (title: string, txt: string) => <span class="menu-title" title={title}>{txt}</span>

    const renderMenuItem = () => {
      const title = onlyOneChild.value.meta?.title || '';
      return <ElMenuItem class={[!props.isNest && 'submenu-title-noDropdown']} index={resolvePath(onlyOneChild.value.path)} v-slots={{
    ...
    const renderSubMenu = () => <ElSubMenu ref="subMenu" index={resolvePath(props.item.path)} popperAppendToBody v-slots={{
      default: () => props.item.children.map((child: LayoutRoutes, index: number) =>
          (<SidebarItem class="nest-menu" key={child.path + index} item={child} basePath={resolvePath(child.path)} isNest={true} />)),
      title: () => props.item.meta && (<>
    ...
    const renderChild = () => {
      if (hasOneShowingChild(props.item.children, props.item as LayoutRoutes) && (!onlyOneChild.value.children || onlyOneChild.value.noShowingChildren) && !props.item.alwaysShow) {
        if (onlyOneChild.value.meta) {
          return (<AppLink to={resolvePath(onlyOneChild.value.path, onlyOneChild.value.query || '')} v-slots={{
